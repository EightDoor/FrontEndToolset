    ...
    <el-col :span="12">
      <div class="select-btn">
        <div class="select-show">
          <span
            :style="selectIndex[0] === 0 ? { color: 'blue' } : { color: 'red' }"
            >{{ options[selectIndex[0]].label }}</span
          >
          <el-icon><Switch /></el-icon>
          <span
            :style="selectIndex[0] === 0 ? { color: 'red' } : { color: 'blue' }"
            >{{ options[selectIndex[1]].label }}</span
          >
        </div>
      </div>
    </el-col>
  ...
  <el-row>
    <el-col :span="11">
      <div class="imageContent__clear">
        <el-button type="primary" @click="changeNewSelect"> 切换 </el-button>
        <el-button
          type="success"
          size="large"
        ...
        <el-button type="primary" plain @click="clearText"> 清空 </el-button>
      </div>
      <el-input
        ref="contentImgRef"
    ...
    <el-col :span="12" class="right_content">
      <div>
        ...
        <div class="tw-flex tw-flex-row tw-items-center">
          ...
          <img
            v-if="data.resultText.length > 0"
            class="tw-w-5 tw-h-5 speak-img tw-ml-4"
            src="/images/speak.png"
            @click="pronunciationFun"
          />
        </div>
      </div>
      <div v-if="selectIndex[0] === 0">
        <el-divider />
          ...
          <template v-for="(item, index) in defaultList">
            <li v-if="formatTxt(item)" :key="index">
              ...
              <el-button
                ...
                type="success"
                class="copy"
                :data-clipboard-text="formatTxt(item)"
                @click="copyData"
              >
                复制
              </el-button>
...
function clearText() {
  data.entryText = "";
  data.resultText = [];
  if (contentImgRef.value) (contentImgRef.value as HTMLElement).innerHTML = "";
}
